...
#[cfg(target_arch = "aarch64")]
use devices::gic::{create_vgic_config, GIC_V3_ITS_SNAPSHOT_ID};
#[cfg(target_arch = "aarch64")]
...
pub enum Error {
    ...
    #[error("Power button not supported: {0:?}")]
    PowerButtonNotSupported(#[source] vmm_sys_util::errno::Error),
...
impl Vm {
    ...
    fn new_from_memory_manager(
        ...
        let load_payload_handle = if !restoring {
            Self::load_payload_async(&memory_manager, &config, device_manager.clone())?
        } else {
            None
        };
        ...
        config
            ...
            .map(|payload| {
                ...
                std::thread::Builder::new()
                    ...
                    .spawn(move || Self::load_payload(&payload, memory_manager, &device_manager))
    ...
    fn restore_vgic_and_enable_interrupt(
        ...
        self.device_manager
            ...
            .lock()
            .unwrap()
            .create_vgic(&self.vm, create_vgic_config(vcpu_count))
    ...
    pub fn power_button(&self) -> Result<()> {
        ...
        Err(Error::PowerButtonNotSupported(
            vmm_sys_util::errno::Error::new(1),
        ))
    }
