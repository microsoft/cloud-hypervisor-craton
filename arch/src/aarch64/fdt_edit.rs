...
fn copy_from_FdtTree(dt: &DeviceTree) -> FdtWriterResult<Vec<u8>> {
...
fn find_parent<'a>(dt: &'a mut DeviceTree, name: &str) -> Option<&'a mut device_tree::Node>{
...
fn edit_FdtTree_with_writer(dt: &DeviceTree,node_name: &str, prop_name: &str, value: &Vec<u8> ) -> FdtWriterResult<Vec<u8>> {
...
fn edit_node_with_writer(writer: &mut FdtWriter, node: &device_tree::Node, node_name: &str, prop_name: &str, new_value: &Vec<u8> ) -> FdtWriterResult<()> {
    let child = writer.begin_node(&node.name)?;
    for prop in node.props.iter() {
...
fn modify_prop_regs(dt: &DeviceTree, node_name: &str, prop_name: &str, acells: u32, adress: u64, scells: u32, size:u64) -> FdtWriterResult<Vec<u8>>  {
    ...
    edit_FdtTree_with_writer(dt,node_name,prop_name, &arr)
}
...
fn edit_FdtTree(dt: &mut DeviceTree,node_name: &str, prop_name: &str, value: &Vec<u32> ) -> FdtWriterResult<()> {
...
fn find_node<'a>(dt: &'a mut DeviceTree, name: &str) -> Option<&'a mut device_tree::Node>{
